...
class DeleteHorseTreatment(
    private val horseDetailService: HorseDetailServiceImpl
) {
    fun invoke(horseTreatmentId: String): CommonFlow<DataState<BooleanResponse>> = flow {
        emit(DataState.Loading(true))
        try{
            val response = horseDetailService.removeHorseTreatment(horseTreatmentId)
